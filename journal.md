...
- Changed: 颜值梯度统一显示为 T0～T5，T0 为最高级；曲线纵轴将 T0 置顶，平均梯度显示 T 前缀，自动平均梯度天梯按低数值优先排列。天梯达人卡片只保留姓名，达人索引卡片用同位置、同字号的未来行程替换简介。
- Verified: 152 项 Vitest 通过（另有 10 项历史测试跳过），ESLint 与生产构建通过；本地浏览器确认平均梯度分档、纯姓名天梯卡片及 T0 置顶曲线，无错误覆盖层。
