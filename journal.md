...
- Changed: 地点行程弹窗按达人分为未来、过去两大类；存在任意未来行程的达人归入顶部未来类并按最近未来行程排序。达人卡片内未来行程统一置顶、过期行程沉底；仅当前地点命中的过期行程使用淡红档案底和红色索引脊，其他过期行程保持中性色。
- Verified: 151 项 Vitest 通过（另有 10 项历史测试跳过），ESLint 与生产构建通过。
